...
Public Class LabUtils
    Public Shared Sub ImportLabReal(ByVal fonte As Connection, ByVal location As Int16)
        ...
        With cmmFonte
            ...
            If AllPatients Then
                .CommandText = "Select distinct nid from t_resultadoslaboratorio"
            Else
                .CommandText = "Select distinct nid from t_resultadoslaboratorio where nid in (" & whereQuery & ")"
            End If
    ...
    Public Shared Sub ImportLabs(ByVal fonte As Connection, ByVal locationid As Int16, ByVal nid As String)
        ...
        Try
            ...
            'Dim set1019 As New ArrayList
            Dim set1632 As New ArrayList
            Dim set1633 As New ArrayList
            Dim set1639 As New ArrayList
            'Dim set1640 As New ArrayList
            'Dim set1658 As New ArrayList
            Dim set1723 As New ArrayList
            'Dim set1787 As New ArrayList
            'Dim set1618 As New ArrayList
            ...
            With cmmFonte
                .ActiveConnection = fonte
                .CommandType = CommandType.Text
                .CommandText = "Select codexame,dataresultado,codparametro,resultado from t_resultadoslaboratorio where nid='" & nid & "' order by dataresultado"
                'rs = .Execute
                rs.Open("Select codexame,dataresultado,codparametro,resultado from t_resultadoslaboratorio where dataresultado is not null and nid='" & nid & "' order by dataresultado", fonte, CursorTypeEnum.adOpenDynamic, LockTypeEnum.adLockReadOnly)
                ...
                If Not (rs.EOF And rs.BOF) Then
                    ...
                    If patientID > 0 Then
                        ...
                        While Not rs.EOF
                            ...
                            Else
                                ...
                                cmmDestino.CommandText = "Insert into encounter(encounter_type,patient_id,provider_id,location_id," & _
                                                    "form_id,encounter_datetime,creator,date_created,voided,uuid) values(13," & patientID & ",27," & locationid & "," & _
                                                    "106,'" & dataMySQL(dataInicial) & "',22,now(),0,uuid())"
                                ...
                                ''If set1019.Count > 0 Then
                                ''    obsSet.concept_id = 1019
                                ''    obsGroupId = ObsDAO.insertSet(obsSet)
                                ''    For Each o As Obs In set1019
                                ...
                                If set1639.Count > 0 Then
                                    ...
                                    Next
                                End If
                                ''If set1640.Count > 0 Then
                                ''    obsSet.concept_id = 1640
                                ''    obsGroupId = ObsDAO.insertSet(obsSet)
                                ''    For Each o As Obs In set1640
                                ''        o.obs_group_id = obsGroupId
                                ''        o.encounter_id = encounter_id
                                ''        ObsDAO.insertObs(o, True)
                                ''    Next
                                ''End If
                                ''If set1658.Count > 0 Then
                                ''    obsSet.concept_id = 1658
                                ''    obsGroupId = ObsDAO.insertSet(obsSet)
                                ''    For Each o As Obs In set1658
                                ''        o.obs_group_id = obsGroupId
                                ''        o.encounter_id = encounter_id
                                ''        ObsDAO.insertObs(o, True)
                                ''    Next
                                ...
                                If set1723.Count > 0 Then
                                    ...
                                    Next
                                End If
                                ''If set1787.Count > 0 Then
                                ''    obsSet.concept_id = 1787
                                ''    obsGroupId = ObsDAO.insertSet(obsSet)
                                ''    For Each o As Obs In set1787
                                ''        o.obs_group_id = obsGroupId
                                ''        o.encounter_id = encounter_id
                                ''        ObsDAO.insertObs(o, True)
                                ''    Next
                                ''End If
                                ''If set1618.Count > 0 Then
                                ''    obsSet.concept_id = 1618
                                ''    obsGroupId = ObsDAO.insertSet(obsSet)
                                ''    For Each o As Obs In set1618
                                ''        o.obs_group_id = obsGroupId
                                ''        o.encounter_id = encounter_id
                                ''        ObsDAO.insertObs(o, True)
                                ''    Next
                        ...
                        cmmDestino.CommandText = "Insert into encounter(encounter_type,patient_id,provider_id,location_id," & _
                                            "form_id,encounter_datetime,creator,date_created, voided,uuid) values(13," & patientID & ",27," & locationid & "," & _
                                            "60,'" & dataMySQL(dataInicial) & "',22,now(),0,uuid())"
                        cmmDestino.ExecuteNonQuery()
                        'get last inserted encounter id, for use in obs's
                        cmmDestino.CommandText = "Select max(encounter_id) from encounter"
                        encounter_id = cmmDestino.ExecuteScalar
